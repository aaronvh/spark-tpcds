...
case class Run(name: String, description: String, executionDateTime: Long, sparkConfig: Map[String, String]) extends SharedSparkSession {
  def execute(args: Args): RunResult = {
    ...
    val result = RunResult(this, queries.map(query => (query.id, query.execute(args))).toMap)
...
case object Run extends SharedSparkSession {
  ...
  def execute(args: Args): Unit = {
    ...
    val run = Run(argsName, description, executionDateTime, spark.conf.getAll)
  ...
  def compare(args: Args): Unit = {
    println("COMPARE -> TODO!!!")
  }
}
